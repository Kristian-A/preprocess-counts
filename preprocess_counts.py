...
def matched_features(t2g):
    ret = []
    with open('cells_x_genes.genes.txt', 'r') as features_file:
...
# t2g = load_t2g('t2g.txt')
# features = matched_features(t2g)
# save_features(features)

def transpose_matrix(path):
    with open(path, 'r') as matrix, open('tr_matrix.mtx', 'w') as transposed:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mtx', type=str, help='Path to matrix file', required=True)
